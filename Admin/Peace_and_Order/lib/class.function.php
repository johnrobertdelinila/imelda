...
class allfunctions extends database{
    ...
    public function insertIncident($data){
        ...
        $query = 'INSERT into ms_incident (blotterType_id,date_incident, case_incident, time_incident, location,  narrative, settlement, incident_title, date_reported, forwarded) values ("'.$data['blotter_type'].'","'.$date.'","'.$data['incident_type'].'","'.$hour_24.'","'.$data['incident_location'].'","'.$data['narrative'].'","'.$data['settlement'].'","'.$data['incident_title'].'","'.$dateNow.'","'.$data['forwarded'].'")';
    ...
    public function updateIncident($data){
        ...
        $date = date('Y-m-d', $time);
        $hour_24  = date("H:i:s", strtotime(strtoupper($data['time'])));
        $query = "UPDATE ms_incident SET case_incident = '".$data['incident_type']."', blotterType_id = '".$data['blotter_type']."', status = '".$data['status']."', date_incident = '". $date ."', time_incident = '".$hour_24."', location = '".$data['incident_location']."',  narrative = '".$data['narrative']."', settlement = '".$data['settlement']."', incident_title = '".$data['incident_title']."', date_reported = '".$dateNow."', forwarded = '".$data['forwarded']."' WHERE incident_id = '".$data['id']."' ";
