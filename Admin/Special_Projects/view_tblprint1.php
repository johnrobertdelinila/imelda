...
// add a page
$pdf->AddPage('L');
  $pdf->Image('banaba.png',68,9,30);
 $pdf->Image('indang-logo.png',200,9,25); 
$print=$_POST['print'];
if($print == "All")
{
	$query = "SELECT * from annual_project WHERE `source` = 'Sangguniang Kabataan Fund'";
...
{ 
	$query = "SELECT * from annual_project WHERE `source` = 'Sangguniang Kabataan Fund' and (start like '%$print%' or end like '%$print%')";	
	$query1 = "SELECT SUM(amount) from annual_project where `source` ='Sangguniang Kabataan Fund' and (start like '%$print%' or end like '%$print%')";
...
$total = $_SESSION['total'];
    ...
    <tr><td>SANGGUNIANG KABATAAN FUND</td></tr>
    <tr><td>Details of Program/ Project/ Activity by Sector.</td></tr>
	<tr><td>Budget Year: '.$print.'</td></tr>
	<tr><td>Municipality of Naguilian</td></tr>
	<tr><td>Province of La Union</td></tr>
</table>
';
			$html2= '<table align="center"  cellspacing="auto" cellpadding="auto" border="1">
			<tr>
