...
if($print=="All")
{
	$query = "SELECT * from annual_project where `source` ='Senior Citizen Persons with Disability Fund'";	
...
{
	$query = "SELECT * from annual_project WHERE `source` ='Senior Citizen Persons with Disability Fund' and (start like '%$print%' or end like '%$print%')";	
	$query1 = "SELECT SUM(amount) from annual_project where `source` ='Senior Citizen Persons with Disability Fund' and (start like '%$print%' or end like '%$print%')";
...
else
{
    ...
    <tr><td>SENIOR CITIZEN PERSONS WITH DISABILITY FUND</td></tr>
